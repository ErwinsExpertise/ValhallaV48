...
if (plr.job() !== 300 || plr.getLevel() < 30) {
    npc.sendOk("If Athena Pierce did not send you here, then this test is not for you.");
} else if (!plr.haveItem(LETTER, 1)) {
    npc.sendOk("Go back to Athena Pierce in Henesys first.");
} else if (plr.itemCount(MARBLE) > 0) {
    if (!npc.sendYesNo("So you gave up once already. That's fine. I can let you try again, but I need to remove the Dark Marbles you collected first. Do you want to go back in?")) {
