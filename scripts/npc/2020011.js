...
function confirmAdvance() {
    if (!npc.sendYesNo("You've passed the tests. Make sure you've used all of the SP you've earned up to level 70 first. Do you want to become a 3rd job Thief now?")) {
    ...
    var jobId = plr.job() === 410 ? 411 : 421;
    plr.setJob(jobId);
    plr.giveSP(1);
    plr.giveAP(5);
    npc.sendOk("Excellent. You have taken the next step as a Thief.");
}

if (plr.job() !== 410 && plr.job() !== 420 && plr.job() !== 411 && plr.job() !== 421) {
    npc.sendOk("I'm Arec, chief of all Thieves. Speak to your own class chief if you need guidance.");
} else if (plr.job() === 411 || plr.job() === 421) {
    npc.sendOk("You already passed my tests. Keep your edge sharp.");
    ...
    if (!npc.sendYesNo("I am Arec, chief of all Thieves. Do you want to take the test for the 3rd job advancement?")) {
        npc.sendOk("Come back when you're truly ready.");
    } else {
        plr.startQuest(QUEST);
        plr.setQuestData(QUEST, "s");
        npc.sendOk("Go to #b#p1052001##k in Kerning City and complete the physical test first. Bring back #b#t4031057##k when you have passed it.");
    }
} else if (plr.questData(QUEST) === "s" || plr.questData(QUEST) === "p1") {
    npc.sendOk("You need to pass the Dark Lord's physical test first. Bring me #b#t4031057##k.");
} else if (plr.questData(QUEST) === "p2") {
    if (!plr.haveItem(STRENGTH_NECKLACE, 1)) {
        npc.sendOk("Go back to the Dark Lord and get #b#t4031057##k first.");
    } else {
