...
function buildPrompt(recipe, qty, note) {
...
if (type === 0) {
    ...
    recipe = [
        { item: 4011007, mats: [4011000, 4011001, 4011002, 4011003, 4011004, 4011005, 4011006], qty: [1, 1, 1, 1, 1, 1, 1], cost: 10000 },
        { item: 4021009, mats: [4021000, 4021001, 4021002, 4021003, 4021004, 4021005, 4021006, 4021007, 4021008], qty: [1, 1, 1, 1, 1, 1, 1, 1, 1], cost: 15000 }
    ][sel2];
    qty = npc.sendNumber("So, you want me to make some #t" + recipe.item + "#s? In that case, how many do you want me to make?", 1, 1, 100);
} else if (type === 3) {
    var sel3 = menu("Crystal ore? It's hard to find those around here...", ["Power Crystal", "Wisdom Crystal", "DEX Crystal", "LUK Crystal", "Dark Crystal"]);
    recipe = [
        { item: 4005000, mats: [4004000], qty: [10], cost: 5000 },
        { item: 4005001, mats: [4004001], qty: [10], cost: 5000 },
        { item: 4005002, mats: [4004002], qty: [10], cost: 5000 },
        { item: 4005003, mats: [4004003], qty: [10], cost: 5000 },
        { item: 4005004, mats: [4004004], qty: [10], cost: 1000000 }
    ][sel3];
    qty = npc.sendNumber("So, you want me to make some #t" + recipe.item + "#s? In that case, how many do you want me to make?", 1, 1, 100);
} else if (type === 4) {
...
if (!npc.sendYesNo(buildPrompt(recipe, qty))) {
    ...
    npc.sendOk("I'm afraid you don't have a slot available for the item in your inventory.");
} else if (plr.getMesos() < totalCost) {
    npc.sendOk("I'm afraid you cannot afford my services.");
} else if (!hasMaterials(recipe, qty)) {
    npc.sendOk("I can't refine anything for you without the proper items.");
} else {
    ...
    npc.sendOk("All done. If you need anything else, just ask.");
}
